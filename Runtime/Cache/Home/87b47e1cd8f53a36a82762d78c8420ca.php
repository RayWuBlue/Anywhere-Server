...
<DIV class=PleftM>
<DIV class=rcur>
<P><STRONG>当前位置：</STRONG><A href="__APP__/index/index">首页</A>&gt;<strong>信息列表</strong></P>
</DIV>
<DIV class=tit01>
<DIV class=Ptit01T1><IMG src="Public/images/d10.gif"> 
<H2><strong>信息列表</strong></H2>
</DIV>
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
	  ...
	  <TR class=BGWhite vAlign=top>
          <TD align=middle>
		  分类查看：&nbsp;
		  <select name="ttedu_id" onChange="window.open('/index.php?s=/InfoDetail/infolist/ttedu_id/'+this.options[this.selectedIndex].value,'_self')">
		  <?php if(is_array($btype_info)): $i = 0; $__LIST__ = $btype_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><option value="<?php echo ($vo["ttedu_id"]); ?>" <?php if(($vo["ttedu_id"])  ==  $ttedu_id): ?>selected<?php endif; ?> ><?php echo ($vo["ttedu_btype_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		  </select>
		  </TD></TR>
        <TR>
...
<?php if(is_array($info_detail)): $i = 0; $__LIST__ = $info_detail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><LI>
<A href="__APP__/InfoDetail/infodetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></A></LI><?php endforeach; endif; else: echo "" ;endif; ?>

 
</OL><BR>
<DIV class=num>
<?php echo ($page); ?>
</DIV></DIV></DIV><!--中部左区结束，右区开始-->
<DIV class=PrightM>
<DIV class=tit01>
<DIV class=zxtit01T2>
